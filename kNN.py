...
if __name__ == '__main__':
    ...
    k = int(math.sqrt(train_fea.shape[0]))
    if (k % 2 == 0):
        k -= 1
    test_lab_pred = KNN_classifier(train_fea, train_lab, test_fea, k)
    # calculate accuracy
    acc = calculate_accuracy(test_lab_pred, test_lab_true)
    print('The accuracy of kNN algorithm for case of breast cancer wisconsin is', acc)
    # draw confusion matrix
    plot_confusion_matrix(test_lab_pred, test_lab_true, ['benign','malignant'])
    ...
    print('The accuracy of kNN algorithm for case of breast cancer wisconsin is', acc)
    # draw confusion matrix
    plot_confusion_matrix(test_lab_pred, test_lab_true, ['survived >= 5 years', 'died within 5 years'])
